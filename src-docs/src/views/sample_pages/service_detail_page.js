...
const TOP_DEPENDENCIES = [{ service: 'checkout', faultRate: 66.67 }];
    ...
    <svg
      ...
      {[0, 2, 4, 6, 8, 10].map((v) => (
        <line
          key={v}
          x1={0}
          y1={height - (v / 10) * height}
          x2={width}
          y2={height - (v / 10) * height}
          stroke="#E4EAF2"
          strokeWidth={1}
        />
      ))}
      ...
      <line
        x1={0}
        y1={height - (p99 / 10) * height}
        x2={width}
        y2={height - (p99 / 10) * height}
        stroke="#ED6F73"
        strokeWidth={2}
      />
      {/* P90 line (yellow) */}
      <line
        x1={0}
        y1={height - (p90 / 10) * height}
        x2={width}
        y2={height - (p90 / 10) * height}
        stroke="#CDA849"
        strokeWidth={2}
      />
      {/* P50 line (teal) */}
      <line
        x1={0}
        y1={height - (p50 / 10) * height}
        x2={width}
        y2={height - (p50 / 10) * height}
        stroke="#5CB198"
        strokeWidth={2}
      />
    </svg>
...
export const ServiceDetailPage = ({
  ...
  const handleViewCorrelatedLogs = () => {
    ...
    if (onQueryExecute) {
      setTimeout(() => {
        onQueryExecute(
          'source = logs | where service = "checkout" | where status >= 500 | stats count() by message'
        );
      }, 500);
    }
  };
            ...
            {CORRELATED_LINKS.map((link) => (
              <div
                ...
                <OuiLink
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    if (link.label === 'View correlated logs')
                      handleViewCorrelatedLogs();
                  }}>
                  {link.label}
                </OuiLink>
              </div>
          ...
          <OuiFlexItem key={stat.label}>
            <OuiPanel
              ...
              <OuiText size="xs" color="subdued">
                <p style={{ margin: 0 }}>
                  {stat.label} {stat.sublabel && <span>· {stat.sublabel}</span>}
                </p>
              </OuiText>
              <OuiSpacer size="s" />
              <OuiTitle size="m">
                ...
                <OuiText size="xs" color="subdued">
                  ...
                  {stat.change && (
                    <span
                      style={{
                        marginLeft: 4,
                        color: stat.color === 'danger' ? '#ED6F73' : '#5CB198',
                      }}>
