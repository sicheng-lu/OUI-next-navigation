...
export const EuiButtonGroup: FunctionComponent<Props> = ({
  ...
  const classes = classNames(
    ...
    `euiButtonGroup${colorToClassNameMap[resolvedColor]}`,
    {
      ...
      'euiButtonGroup--vertical': orientation === 'vertical',
    },
    className
  );
...
// @internal
export type EuiSmallButtonGroupProps = CommonProps & {
  isDisabled?: boolean;
  /**
   * Expands the whole group to the full width of the container.
   * Each button gets equal widths no matter the content
...
type SmallProps = Omit<
  ...
  'onChange' | 'color'
> &
  EuiSmallButtonGroupProps;

// @internal
export const EuiSmallButtonGroup: FunctionComponent<SmallProps> = (props) => (
  <EuiButtonGroup {...props} buttonSize="s" />
);

// @internal
export type EuiCompressedButtonGroupProps = EuiSmallButtonGroupProps;

// @internal
export const EuiCompressedButtonGroup: FunctionComponent<SmallProps> = (
  props
