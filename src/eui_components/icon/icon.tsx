...
function isEuiIconType(type: EuiIconProps['type']): type is EuiIconType {
  return typeof type === 'string' && typeToPathMap.hasOwnProperty(type);
}

function isUrl(type: EuiIconProps['type']) {
  return typeof type === 'string' && (type.includes('.') || type.includes('/'));
}

function isCachedIcon(type: EuiIconProps['type']) {
  return isEuiIconType(type) && iconComponentCache.hasOwnProperty(type);
}
...
export class EuiIcon extends PureComponent<EuiIconProps, State> {
  ...
  constructor(props: EuiIconProps) {
    ...
    // Category 1: cached eui icons
    if (isCachedIcon(type)) {
      initialIcon = iconComponentCache[type as string];
      // Category 2: URL (relative, absolute)
    } else if (isUrl(type)) {
      initialIcon = type;
      // Category 3: non-cached eui icon or new icon
    } else if (typeof type === 'string') {
      isLoading = true;
      this.loadIconComponent(type as EuiIconType);
    } else {
