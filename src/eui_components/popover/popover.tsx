...
export class EuiPopover extends Component<Props, State> {
  ...
  onChildMouseDown = (event: React.MouseEvent) => {
    const euiEvent = (event.nativeEvent as unknown) as EuiEvent;
    if (euiEvent.hasOwnProperty('euiGeneratedBy')) {
      euiEvent.euiGeneratedBy.push(this.popoverId);
    } else {
      euiEvent.euiGeneratedBy = [this.popoverId];
    }
  };
  ...
  setupBackupClickDetection = () => {
    ...
    this.handleDocumentClick = (event: MouseEvent) => {
      ...
      const euiEvent = (event as unknown) as EuiEvent;
      ...
      const isFromPopoverChild =
        (euiEvent.euiGeneratedBy &&
          euiEvent.euiGeneratedBy.includes(this.popoverId)) ||
