...
export interface EuiOllyChatPillProps
  extends CommonProps,
...
export const EuiOllyChatPill: FunctionComponent<EuiOllyChatPillProps> = ({
  ...
  const classes = classNames(
    'euiOllyChatPill',
    {
      'euiOllyChatPill--expanded': isExpanded || !!message || isDismissing,
      'euiOllyChatPill--highlighted': isHighlighted,
      'euiOllyChatPill--dismissing': isDismissing,
    },
    className
  );
  ...
  return (
    <div
      ...
      {message && !isDismissing && (
        <div
          className="euiOllyChatPill__message"
          onClick={() => onActivate && onActivate()}>
          <p className="euiOllyChatPill__messageText">{message}</p>
          {onDismiss && (
            <EuiButtonIcon
              iconType="cross"
              aria-label="Dismiss"
              size="xs"
              color="text"
              className="euiOllyChatPill__dismiss"
      ...
      {message && !isDismissing && quickReplies && quickReplies.length > 0 && (
        <div className="euiOllyChatPill__quickReplies">
          {quickReplies.map((reply, i) => (
            <button
              key={i}
              type="button"
              className={`euiOllyChatPill__quickReply${
                reply.primary ? ' euiOllyChatPill__quickReply--primary' : ''
              }`}
              onClick={(e) => {
                e.stopPropagation();
      ...
      <div className="euiOllyChatPill__inputRow">
        {avatar && (
          <EuiToolTip content="Open chat" position="top" delay="long">
            <button
              type="button"
              className={`euiOllyChatPill__avatar${
                isHighlighted || isExpanded
                  ? ' euiOllyChatPill__avatar--highlight'
                  : ''
              }${
                !isExpanded && !isHighlighted
                  ? ' euiOllyChatPill__avatar--static'
                  : ''
              }`}
        ...
        <input
          ref={inputRef}
          type="text"
          className="euiOllyChatPill__input"
...
EuiOllyChatPill.displayName = 'EuiOllyChatPill';
